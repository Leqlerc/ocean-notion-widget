...
- `api/events.py` → `lib/calendar.py`: events only. Google and Notion events load concurrently, merge conservatively across sources, and sort by actual start instant. Each event retains source metadata; partial outages are explicitly reported. The original `api/calendar-data.py` and old widgets remain available.
