...
- `api/tasks.py` → `lib/tasks.py`: normalized TaskStore, backed by Tasks master. GET lists tasks and editing options; POST creates; PATCH updates a task; DELETE moves it to recoverable Notion Trash. Notion transport and server IDs live in `lib/notion.py`. Tasks use a `Projects` relation for persistent project membership; legacy `Project` text remains compatible. Completion stamps Date Completed; reopening clears it.
- `api/events.py` → `lib/calendar.py`: events only. Live Google is authoritative for calendar events. Home excludes all Notion Google mirrors and unknown-origin Notion events, even during Google outages. Only independently sourced HTTPS Purdue academic entries remain eligible from Notion; these merge conservatively and sort by actual start instant. Each event retains source metadata; partial outages are explicitly reported. The original `api/calendar-data.py` and old widgets remain available.
...
The existing `NOTION_TOKEN` continues to be required. Optional server overrides are `NOTION_TASKS_DATA_SOURCE_ID` and `NOTION_EVENTS_DATA_SOURCE_ID`. Existing deployments use the current server-side defaults. Tasks master retains `Project` text and now has a dual `Projects` relation. No existing views were changed.

For direct Google Calendar, configure **all three** Vercel environment variables `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, and `GOOGLE_REFRESH_TOKEN`, from one Google OAuth connection with the `https://www.googleapis.com/auth/calendar.readonly` scope. Optional `GOOGLE_CALENDAR_IDS` is a comma-separated list; defaults are primary, Purdue Classes and Class Deadlines, using the existing calendar identifiers server-side. Explicit IDs override those defaults. The connection only needs to read events; no CalendarList discovery scope is required. Per-calendar failures omit that calendar and retain successful sources. Redeploy after configuration. The refresh token must be durable for ongoing access (Google consent apps left in external Testing may issue expiring refresh tokens). No OAuth UI is implemented. Without Google configuration, only clearly independent Purdue coursework and owner-scoped Outlook events can appear. Google failures do not silently switch to an older source labeled as direct.
...
`calendar-semantics.js` classifies events once for both the list and calendar. Priority: exam, assessment, presentation, personal, break, class. Each date exposes event names via title/ARIA. `DirectGoogleCalendarProvider` refreshes/caches an access token, paginates recurring occurrences across configured calendars, and ignores cancelled events. `/api/events` exposes `direct`, `source`, and missing configuration variable names without secrets. The full snapshot makes edits/removals appear on the next 45-second refresh. Notion Google mirrors never enter Home results. Remaining independent Purdue coursework is still read from Notion until a later migration; legacy widget endpoints remain available but are not read by Home.
...
## Coordinated dashboard improvements

`api/projects.py` → `lib/projects.py` loads an independent Projects data source (`8772813a-3a37-49d5-8719-c508aca035e6`, optionally overridden by `NOTION_PROJECTS_DATA_SOURCE_ID`). No Projects database existed, so a minimal database was created under the existing Data vault with Name, Status (Active/Completed/Archived), and a dual Tasks relation. The two existing named task projects were seeded and linked. Active zero-task and fully completed projects remain visible; only project status hides them. Include inactive allows restoration. Progress counts every related task category. Legacy text membership is used only when a task has no relation.

`nocean-deadlines.js` serializes separate date/time controls with the browser's local offset on the chosen day, including daylight saving changes. A blank time means 23:59; explicit times survive. Editing another task field leaves the original deadline untouched. Direct API callers sending date-only values default to 23:59 in the existing campus timezone.

`nocean-motion.js` updates keyed task rows, preserves scroll/focus, and uses brief checked feedback followed by a 240 ms collapse. The existing toast offers six-second Undo. Pending writes and revisions suppress duplicate actions and stale refreshes. Reduced-motion users skip row animations. `nocean-interactions.css` increases Today useful height, uses native overflow scrolling, and removes blur from the scrolling card. The obsolete task-linger script is no longer loaded.
