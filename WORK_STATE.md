...
- Deadlines now defaults to one chronological due-date list across classes. A Sort control can switch to the existing class-grouped accordion view; class mode retains its local collapse and item-count preferences. Submission confirmation remains separate from task completion.
- Home desktop proportions now favor Tasks over Deadlines (1.35fr vs 1.2fr), while the Events/Habitat side column is constrained to the same 620px desktop height so the Habitat bottom aligns with Tasks and Deadlines. Cove Tree now uses the newly supplied artwork.
