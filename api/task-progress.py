...
def formula_value(page, name):
    formula = prop(page, name).get("formula") or {}
    kind = formula.get("type")
    if kind:
        return formula.get(kind)
...
def build_progress():
    ...
    # If a task was just marked Done in the active Today/Focus scope, stamp its
    # completion date immediately. Also clear stale Focus from older completions.
    for page in pages:
...
class handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        try:
            self.send_json(200, build_progress())
        except Exception as exc:
            # Do not expose the token; API/permission errors are safe and useful for debugging.
