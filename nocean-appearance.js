...
const NOceanAppearance=(()=>{
  ...
  const assets={'none':'None / solid','safe-shallows':'Safe Shallows','kelp-forest':'Kelp Forest','grassy-plateaus':'Grassy Plateaus',jellyshroom:'Jelly Shroom','grand-reef':'Grand Reef','blood-kelp':'Blood Kelp','bulb-zone':'Bulb Zone','lost-river':'Lost River','tree-cove':'Tree Cove'};
  ...
  function validate(value,fallback='none') {
    ...
    return {image:Object.hasOwn(assets,v.image)?v.image:fallback,visibility:Object.hasOwn(visibility,v.visibility)?v.visibility:'Medium',tint:Object.hasOwn(tints,v.tint)?v.tint:'Ocean'};
  }
  ...
  function apply(){
    ...
    for(const el of document.querySelectorAll('[data-cosmetic]')){
      ...
      button.hidden=false;
      if(enabled)paint(el,validate(settings[id],el.dataset.defaultArt||'none'));
  ...
  function values(){return {image:document.getElementById('artImage').value,visibility:document.getElementById('artVisibility').value,tint:document.getElementById('artTint').value};}
  ...
  function open(id){
    ...
    active=id;const value=validate(settings[id],target.dataset.defaultArt||'none');
  ...
  function setup(){
    ...
    dialog.innerHTML=`<form id="appearanceForm"><div class="card-head"><h2 id="appearanceTitle">Appearance</h2><button type="button" class="quiet" id="closeAppearance" aria-label="Close appearance">✕</button></div><div id="artPreview" class="art-preview"><strong id="artPreviewName"></strong><span>Keep the next useful thing in view.</span></div><label>Background image<select id="artImage">${Object.entries(assets).map(([key,name])=>`<option value="${key}">${name}</option>`).join('')}</select></label><div class="form-pair"><label>Art visibility<select id="artVisibility"><option>Subtle</option><option>Medium</option><option>Strong</option></select></label><label>Color treatment<select id="artTint">${Object.keys(tints).map(name=>`<option>${name}</option>`).join('')}</select></label></div><p class="section-note">Saved for this card on this browser. Card Art uses these choices; Default keeps its original appearance.</p><p id="artStatus" role="status"></p><div class="dialog-actions"><button type="button" id="resetAppearance" class="quiet">Reset card</button><button type="submit" class="primary">Save appearance</button></div></form>`;
