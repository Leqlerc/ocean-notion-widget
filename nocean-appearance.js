...
const NOceanAppearance=(()=>{
  ...
  function setup(){
    ...
    dialog.innerHTML=`<form id="appearanceForm"><div class="card-head"><h2 id="appearanceTitle">Appearance</h2><button type="button" class="quiet" id="closeAppearance" aria-label="Close appearance">✕</button></div><div id="artPreview" class="art-preview"><strong id="artPreviewName"></strong><span>Trust the process. Own the inputs.</span></div><label>Background image<select id="artImage">${Object.entries(assets).map(([key,name])=>`<option value="${key}">${name}</option>`).join('')}</select></label><div class="form-pair"><label>Art visibility<select id="artVisibility"><option>Subtle</option><option>Medium</option><option>Strong</option></select></label><label>Color treatment<select id="artTint">${Object.keys(tints).map(name=>`<option>${name}</option>`).join('')}</select></label></div><p class="section-note">Saved for this card on this browser. Cards use these choices in Card Art. The welcome banner uses them in every preset.</p><p id="artStatus" role="status"></p><div class="dialog-actions"><button type="button" id="resetAppearance" class="quiet">Reset card</button><button type="submit" class="primary">Save appearance</button></div></form>`;
