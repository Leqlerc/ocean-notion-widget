...
const NOceanUpkeep=(()=>{
  ...
  if($('upkeepForm')){
    ...
    $('upkeepForm').onsubmit=e=>{e.preventDefault();try{const items=NOceanStore.settings().maintenance,id=$('upkeepId').value||crypto.randomUUID(),entry={id,name:$('upkeepName').value.trim(),mode:$('upkeepMode').value,every:Number($('upkeepEvery').value),weekday:Number($('upkeepWeekday').value),start:$('upkeepStart').value};if(!entry.name)return;const i=items.findIndex(x=>x.id===id);if(i<0)items.push(entry);else items[i]=entry;save(items);reset();render();message('Upkeep saved.');}catch(error){message(error.message);}};
    $('upkeepDue').onchange=e=>{const id=e.target.dataset.upkeepComplete;if(!id)return;try{NOceanStore.completeMaintenance(id,dayKey(),false);render();}catch(error){e.target.checked=false;message(error.message);}};
