...
function renderDining(data) {
  ...
  $('dining').innerHTML=(data.meals?.[diningMeal]||[]).map(court=>{
    ...
    return `<article class="dining-court"><div class="court-top"><h3>${esc(court.name)}</h3><span class="crowd-badge crowd-${esc(level||'unknown')}" title="${esc(crowd.source||'No current crowd report')} · checked ${esc(timeLabel(crowd.checkedAt))}">● ${esc(label||'Crowd unavailable')}${fresh&&crowd.percent!=null?' · '+crowd.percent+'%':''}${fresh&&crowd.estimate?' · ~'+esc(crowd.estimate):''}</span></div><div class="court-meta">${esc(status)}${court.start?' · '+esc(timeLabel(court.start))+'–'+esc(timeLabel(court.end)):''}${court.service&&court.service!==diningMeal?' · '+esc(court.service):''}</div><div>${court.picks.map((p,i)=>`<div class="pick"><span>${esc(p.name)}${i===0&&p.rotating?'<small class="rotating">Rotating pick</small>':''}</span><span class="macros" title="${esc(p.serving||'Serving size unavailable')} · ${esc(p.station||'')}">${p.protein==null?'Nutrition unavailable':p.protein+'g protein'+(p.fat!=null?' · '+p.fat+'g fat':'')}</span></div>`).join('')||empty(court.message||'No protein picks published.')}</div></article>`;
  }).join('');
}
