...
(async()=>{
 ...
 records.push({id:'manual',name:'Manual Friday work',course:'CS 159',priority:'Normal',status:'next',focus:false,planningMode:'automatic',difficulty:'Medium',due:'2026-09-25T23:00:00-04:00'});
 ...
 assert.doesNotMatch($('taskList').textContent,/Today assignment|Overdue assignment/);
 w.document.querySelector('[data-tab="upcoming"]').click();assert.doesNotMatch($('taskList').textContent,/Friday assignment/);assert.match($('taskList').textContent,/Manual Friday work/);
 ...
 w.document.querySelector('[data-tab="upcoming"]').click();w.document.querySelector('[data-edit="manual"]').click();
 $('editName').value='Edited Friday work';w.document.querySelector('[data-plan="tomorrow"]').click();$('editTask').dispatchEvent(new w.Event('submit',{cancelable:true}));await tick();
 assert.equal(records.find(t=>t.id==='manual').due,'2026-09-25T23:00:00-04:00');assert.equal(records.find(t=>t.id==='manual').scheduledFor,'2026-09-23');
 ...
 w.document.querySelector('[data-tab="tomorrow"]').click();assert.match($('taskList').textContent,/Edited Friday work/);
 $('refresh').click();await tick();assert.equal(records.length,8);
