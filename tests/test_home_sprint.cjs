...
const scripts=['nocean-shared.js','nocean-data.js','nocean-deadlines.js','nocean-planning.js','calendar-semantics.js','nocean-store.js'];
(async()=>{
 ...
 records.push({id:'m0',name:'Normal manual',course:'MFET 163',priority:'Normal',status:'next',focus:true,planningMode:'planned',scheduledFor:'2026-09-23'},{id:'m1',name:'Critical manual',priority:'Critical',status:'next',focus:true,planningMode:'planned',scheduledFor:'2026-09-23'},{id:'m2',name:'High manual',priority:'High',status:'next',focus:false,planningMode:'planned',scheduledFor:'2026-09-25'},{id:'m3',name:'Low manual',priority:'Low',status:'next',focus:false,planningMode:'planned',scheduledFor:'2026-09-23'});
 const requests=[];
 ...
 assert.match($('#weather').textContent,/Forecast unavailable/);
 ...
 assert.doesNotMatch($('#taskList').textContent,/Assignment 0/);assert.equal(w.document.querySelectorAll('.focus-section').length,1);assert.match($('.focus-section').textContent,/Normal manual/);
 $('[data-focus="m1"]').click();await tick();assert.equal(records.find(t=>t.id==='m1').focus,true);assert.match($('.focus-section').textContent,/Critical manual/);assert.equal(w.document.querySelectorAll('.task-row.is-focused').length,1);
 $('[data-focus="m1"]').click();await tick();assert.equal(w.document.querySelectorAll('.focus-section').length,0);
 $('#taskSort').value='priority';$('#taskSort').dispatchEvent(new w.Event('change'));assert.equal(JSON.parse(w.localStorage.getItem('nocean.command.settings.v1')).dashboard.taskSort,'priority');
 assert.deepEqual([...w.document.querySelectorAll('#taskList .task-name')].map(x=>x.textContent),['Critical manual','Normal manual','Low manual']);
 $('[data-tab="upcoming"]').click();$('[data-today="m2"]').click();await tick();
 assert.equal(records.find(t=>t.id==='m2').scheduledFor,'2026-09-23');assert.equal(requests.at(-1).due,undefined);assert.equal(requests.at(-1).focus,false);
 ...
 const swatches=v.document.querySelectorAll('[data-accent]');assert.equal(swatches.length,24);assert.equal(v.document.querySelector('#accentColor').tagName,'DIV');
 ...
 settings.window.close();console.log('Home sprint passed: imported/manual separation, limits, exclusive Focus, Priority sort, Today planning, event order, layout preferences, and accents.');
