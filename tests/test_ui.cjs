...
for(const [key,action] of Object.entries({n:'new','/':'new','1':'today','2':'tomorrow','3':'later',r:'refresh','?':'help'}))assert.equal(run(`NOceanUI.shortcutAction({key:${JSON.stringify(key)}},false)`),action);
