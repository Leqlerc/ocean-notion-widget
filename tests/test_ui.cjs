...
const ctx=vm.createContext({localStorage:{setItem:(k,v)=>stored[k]=v},document:{addEventListener:()=>{},body:{classList:{toggle:(k,v)=>v?classes.add(k):classes.delete(k)}},getElementById:()=>({setAttribute:(k,v)=>attributes[k]=v})}});
...
for(const [name,key] of [['MA 261 Exam','exam'],['Quiz','assessment'],['CFU','assessment'],['Practical','assessment'],['Demo','presentation'],['Roundtable','presentation'],['Meetup','personal'],['Fall Break','break'],['Lecture','class']])assert.equal(run(`CalendarSemantics.classify({name:${JSON.stringify(name)}}).key`),key);
assert.equal(run(`CalendarSemantics.classify({name:'Quiz',type:'Class'}).key`),'assessment');
